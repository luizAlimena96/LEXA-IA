...
import { FieldMapping, CRMStage, ZapSignConfigEditorProps } from '@/app/agentes/components/interfaces';
...
interface TemplateZapSignEditorProps {
    templateId: string;
    onUpdate: () => void;
}

export default function TemplateZapSignEditor({ templateId, onUpdate }: TemplateZapSignEditorProps) {
    ...
    const loadConfig = async () => {
        try {
            ...
            const [data, stagesData] = await Promise.all([
                api.get(`/agent-templates/${templateId}/zapsign`),
            ...
            if (configData.zapSignTriggerCrmStageId) {
                setTriggerCrmStageId(configData.zapSignTriggerCrmStageId);
            }
            if (!configData.zapSignFieldMapping || configData.zapSignFieldMapping.length === 0) {
                setFieldMappings([
                ...
                setFieldMappings(configData.zapSignFieldMapping);
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            await api.put(`/agent-templates/${templateId}/zapsign`, {
