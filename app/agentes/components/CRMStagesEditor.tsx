...
import { CRMStage, State, CRMStagesEditorProps, StageFormData } from './interfaces';
import CRMStagesList, { SortableStageItem } from './crm-stages/CRMStagesList';
import CRMStageModal from './crm-stages/CRMStageModal';
...
export default function CRMStagesEditor({ agentId, organizationId }: CRMStagesEditorProps) {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            try {
                api.agents.crmStages.reorder(agentId, {
    ...
    const handleSave = async (data: StageFormData) => {
        try {
            setSaving(true);
            if (editingStage) {
                await api.agents.crmStages.update(agentId, editingStage.id, data);
                showMessage('success', 'Etapa atualizada!');
            } else {
                await api.agents.crmStages.create(agentId, {
                    ...data,
                    order: stages.length,
                });
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await api.agents.crmStages.delete(agentId, id);
